...
def select_provider_and_model(args=None):
    ...
    _has_saved_custom_providers = bool(get_compatible_custom_providers(config))
    if _has_saved_custom_providers:
...
def _remove_custom_provider(config):
    ...
    removed_key = (removed.get("provider_key") or "").strip() if isinstance(removed, dict) else ""
    ...
    if isinstance(legacy, list) and legacy:
        ...
        if isinstance(removed, dict):
            normalized_removed = {
                ...
                "base_url": removed.get("base_url", ""),
                "model": removed.get("model", ""),
            }
        for entry in legacy:
            if removed_from_legacy or not isinstance(entry, dict) or normalized_removed is None:
            ...
            if candidate == normalized_removed:
                removed_from_legacy = True
                continue
            remaining_legacy.append(entry)
        cfg["custom_providers"] = remaining_legacy

    if removed_key:
        providers_dict = cfg.get("providers")
        if isinstance(providers_dict, dict) and removed_key in providers_dict:
            providers_dict = dict(providers_dict)
            providers_dict.pop(removed_key, None)
            if providers_dict:
        ...
        providers_dict = cfg.get("providers")
        normalized_providers = providers_dict_to_custom_providers(providers_dict)
        if isinstance(providers_dict, dict) and normalized_providers:
            target_name = removed.get("name", "") if isinstance(removed, dict) else ""
            target_url = removed.get("base_url", "") if isinstance(removed, dict) else ""
            target_model = removed.get("model", "") if isinstance(removed, dict) else ""
            updated = dict(providers_dict)
            for key, entry in list(updated.items()):
                if not isinstance(entry, dict):
                    continue
                normalized = providers_dict_to_custom_providers({key: entry})
                if not normalized:
                    continue
                candidate = normalized[0]
                if (
                    candidate.get("name", "") == target_name
                    and candidate.get("base_url", "") == target_url
                    and candidate.get("model", "") == target_model
                ):
